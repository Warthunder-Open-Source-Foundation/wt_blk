...
use crate::dxp::DxpError::IndexingFileOutOfBounds;
...
pub fn parse_dxp(file: &[u8]) -> Result<Vec<String>, DxpError> {
    ...
    // Fixed offset at 0x8
    let file_count = bytes_to_offset(&file.get(0x8..(0x8 + size_of::<u32>())).ok_or(IndexingFileOutOfBounds { current_ptr: 0x8, file_size: file.len() })?).unwrap();
    ...
    for _ in 0..file_count {
        let str = CStr::from_bytes_until_nul(
            &file.get(ptr..).ok_or(IndexingFileOutOfBounds {
                current_ptr: ptr,
                file_size: file.len(),
            })?
        )?
