...
use wt_blk::blk::{
	leb128::uleb128,
	nm_file::{decode_nm_file, parse_name_section},
	parser::parse_blk,
	zstd::{decode_zstd, eep},
};

pub fn blk_fat(c: &mut Criterion) {
	let f = fs::read("./samples/section_fat.blk").unwrap();
	c.bench_function("blk fat", |b| {
		b.iter(|| parse_blk(black_box(&f), true, false, None, Rc::new(vec![])))
	});
}
...
pub fn zstd(c: &mut Criterion) {
	...
	c.bench_function("zstd", |b| {
		b.iter(|| decode_zstd(black_box(&f), Rc::new(FrameDecoder::new())))
	});
}
...
pub fn nm_section(c: &mut Criterion) {
	...
	c.bench_function("nm_section", |b| {
		b.iter(|| parse_name_section(black_box(&nm[nm_ptr..(nm_ptr + names_data_size)])))
	});
}
